  ...
  async function processAudioChunk(chunk) {
    ...
    if (bufferedBytes < minSegmentBytes) {
      if (diagnostics) {
        sendStatus({
          state: 'capturing',
          message: `Buffering audio (${Math.round(bufferedBytes / minSegmentBytes * 100)}%).`
        });
      }
...
function normalizeTranscript(data, chunk) {
  if (data && typeof data === 'object' && data.text) {
    return {
      text: data.text,
      speaker: data.speaker || 'Transcription API',
      speakerColor: data.speakerColor || '#d8bfd8'
    };
  }

  return createSimulatedTranscript(chunk);
}

function createSimulatedTranscript(chunk) {
  return {
