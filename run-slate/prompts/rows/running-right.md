Create one horizontal animation strip for Codex pet `slate`, state `running-right`.
...
Identity: same pet in every frame: Compact gray fox mascot, clean silhouette, no text, no shadows, no detached effects.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
